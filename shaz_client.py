...
song_label.place(relx=0.5, rely=0.3, anchor=CENTER)

artist_label = Label(
    right_frame, textvariable=artist_text_var, text="Artist", font="Helvetica 12"
)
artist_label.place(relx=0.5, rely=0.4, anchor=CENTER)
...
release_label.place(relx=0.5, rely=0.6, anchor=CENTER)

button = Button(
    right_frame, text="SHAZAM", font="Helvetica 12 bold", command=call_server
)
...
async def connect():
    ...
    """
    reader, writer = await asyncio.open_connection("192.168.1.10", 8000)
    ...
    metadata_size = int.from_bytes(response_bytes[:2], byteorder="big")
    print(f"Client received {metadata_size} metadata bytes")
